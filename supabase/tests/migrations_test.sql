...
insert into public.vercel_logs (id, "timestamp", level, source, request_id, status_code, path, message, raw)
...
insert into public.vercel_logs (id, "timestamp", level, source, raw)
select id, "timestamp", level, source, raw from public.vercel_logs where id = 'test-1'
on conflict ("timestamp", id) do nothing;

do $$
declare n bigint;
begin
  select count(*) into n from public.vercel_logs where id = 'test-1';
  if n <> 1 then
...
select public.vercel_logs_maintenance(14);

do $$
begin
  if to_regclass('public.vercel_logs_p' || to_char(current_date + 1, 'YYYYMMDD')) is null then
...
select public.vercel_logs_create_partition((current_date - 30)::date);

insert into public.vercel_logs (id, "timestamp", raw)
...
declare n bigint;
begin
  if to_regclass('public.vercel_logs_p' || to_char(current_date - 30, 'YYYYMMDD')) is not null then
    raise exception 'expired partition was not dropped';
  end if;
  select count(*) into n from public.vercel_logs where id = 'test-old';
  if n <> 0 then
...
do $$
begin
  if to_regclass('public.vercel_logs_p' || to_char(current_date, 'YYYYMMDD')) is null then
...
begin
  begin
    perform public.vercel_logs_maintenance(0);
...
begin
  select relrowsecurity into enabled from pg_class where oid = 'public.vercel_logs'::regclass;
  if not enabled then
